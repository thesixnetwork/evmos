...
func (oa *OverrideAccount) ToProtoOverrideAccount() *evmtypes.OverrideAccount {
	// Transform Nonce
	nonce := uint64(0)
	if oa.Nonce != nil {
...
func (so *StateOverride) ToProtoStateOverride() *evmtypes.StateOverride {
	protoSO := &evmtypes.StateOverride{
		Accounts: make(map[string]*evmtypes.OverrideAccount),
	}

	for address, overrideAccount := range *so {
		protoSO.Accounts[address.Hex()] = overrideAccount.ToProtoOverrideAccount()
	}
...
func (bo *BlockOverrides) ToProtoBlockOverride() *evmtypes.BlockOverrides {
